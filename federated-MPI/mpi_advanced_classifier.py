...
cifar10_train_files = ['cifar-10-tf-records/train{}.tfrecords'.format(i)
...
with tf.name_scope('dataset'), tf.device('/cpu:0'):
    ...
    def preprocess(serialized_examples):
        ...
        image = tf.map_fn(lambda img: tf.reshape(tf.decode_raw(img, tf.uint8),
                                                 tf.stack([HEIGHT, WIDTH, CHANNELS])),
                          features['image'], dtype=tf.uint8, name='decode')
        # Cast image
...
with tf.name_scope('loss'):
    base_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y,
                                                                              logits=logits),
                               name='base_loss')
    # Add regularization loss to both relu layers
    regularizer_loss = tf.add_n(
        [tf.nn.l2_loss(v) for v in tf.trainable_variables() if 'relu/kernel' in v.name],
        name='regularizer_loss') * 0.004
...
with tf.name_scope('train'):
    ...
    lr = tf.train.exponential_decay(0.1, global_step, N_BATCHES * EPOCHS_PER_DECAY,
                                    0.1, staircase=True)
    ...
    def after_run(self, run_context, run_values):
        ...
        if (step_value + 1) % N_BATCHES == 0 and COMM.rank == 0:
            print("Epoch {}/{} - loss: {:.4f} - acc: {:.4f}".format(
                int(step_value / N_BATCHES) + 1, EPOCHS, self._total_loss / N_BATCHES,
...
class _FederatedHook(tf.train.SessionRunHook):
    ...
    def _create_placeholders(self):
        ...
        for var in tf.trainable_variables():
            self._placeholders.append(tf.placeholder_with_default(
...
class _InitHook(tf.train.SessionRunHook):
    def after_create_session(self, session, coord):
        session.run(dataset_init_op, feed_dict={
            filename_placeholder: cifar10_train_files, batch_size: BATCH_SIZE,
            shuffle_size: SHUFFLE_SIZE, train_mode: True})
...
if COMM.rank == 0:
    ...
    with tf.Session() as sess:
        ...
        sess.run(dataset_init_op, feed_dict={
            filename_placeholder: cifar10_test_file, batch_size: NUM_TEST_IMAGES,
            shuffle_size: 1, train_mode: False})
